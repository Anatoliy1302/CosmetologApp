...
export const NewBreakScreen = ({ route, navigation }) => {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      await addAppointmentLocal({ type: 'break', title: 'Перерыв', description: description.trim() || `Перерыв ${duration} мин`, date: date.trim(), time: time.trim(), duration: parseInt(duration, 10) || 30 });
  ...
  return (
    <ScrollView style={styles.formContainer} showsVerticalScrollIndicator={false}>
      <SafeAreaView>
        <View style={styles.header}><TouchableOpacity onPress={() => navigation.goBack()}><Text style={styles.backButton}>← Назад</Text></TouchableOpacity><Text style={styles.headerTitle}>Перерыв</Text><View style={{ width: 60 }} /></View>
        <View style={styles.formCard}>
          ...
          {showCalendar && <Calendar onDayPress={(day) => { setDate(day.dateString); setShowCalendar(false); }} markedDates={{ [date]: { selected: true, selectedColor: COLORS.warning } }} theme={{ selectedDayBackgroundColor: COLORS.warning, todayTextColor: COLORS.warning, arrowColor: COLORS.warning }} style={styles.inlineCalendar} />}
          <Text style={styles.label}>⏰ Время начала *</Text><TimeSelector value={time} onSelect={setTime} />
          <Text style={styles.label}>⏱ Длительность (минут)</Text><TextInput style={styles.input} value={duration} onChangeText={setDuration} placeholder="30" keyboardType="numeric" placeholderTextColor={COLORS.textLight} />
          <Text style={styles.label}>📄 Примечание</Text>
          <TextInput style={[styles.input, styles.textArea]} value={description} onChangeText={setDescription} placeholder="Например: Обед" multiline numberOfLines={2} placeholderTextColor={COLORS.textLight} />
          <TouchableOpacity style={[styles.saveButton, { backgroundColor: COLORS.warning }]} onPress={save} disabled={saving}><Text style={styles.saveButtonText}>{saving ? 'Сохранение...' : '✓ Сохранить'}</Text></TouchableOpacity>
        </View>
      </SafeAreaView>
    </ScrollView>
  );
